<template>
  ...
  >
    <i class="delete-icon" v-if="showClose" @mousedown.stop="handleDelete"></i>
    <span class="vdr-order" v-if="order">{{order}}</span>
    <div
...
export default {
  ...
  methods: {
    ...
    getParentSize () {
      if (this.parent === true) {
        if (this.$parent.$el.classList.contains('vdr')) {
          return [
            this.$parent.width || this.$parent.w,
            this.$parent.height || this.$parent.h
          ]
        } else {
    ...
    calcDragLimits () {
      if (!this.$children.length) {
        const uniqueKey = this.$parent.uniqueKey
        const parentArray = document.querySelectorAll(`[data-uniqueKey="${uniqueKey}"]`)
        if (parentArray.length > 1) {
          const parentEle = this.$parent
          const originEle = this.$parent.$el.parentElement
          return {
            minLeft: ~parentEle.left,
            maxLeft: originEle.clientWidth - this.width - parentEle.left,
            ...
            minTop: ~parentEle.top,
            maxTop: originEle.clientHeight - this.height - parentEle.top,
    ...
    deselect (e) {
      ...
      // 点击自身（包括控制柄，不包括子元素）
      const selfClick = this.$el === target || target.classList.contains('handle')

      if (!selfClick && !regex.test(target.className)) {
    ...
    // 键盘移动
    async keyboardMove (e) {
      const keyCode = e.keyCode
      switch (keyCode) {
        case 37:
          this.left = restrictToBounds(--this.left, this.bounds.minLeft, this.bounds.maxLeft)
          break
        case 38:
          this.top = restrictToBounds(--this.top, this.bounds.minLeft, this.bounds.maxLeft)
          break
        case 39:
          this.left = restrictToBounds(++this.left, this.bounds.minLeft, this.bounds.maxLeft)
          this.right = restrictToBounds(++this.right, this.bounds.minRight, this.bounds.maxRighminRight)
          break
        case 40:
          this.top = restrictToBounds(++this.top, this.bounds.minTop, this.bounds.maxTop)
          this.bottom = restrictToBounds(++this.bottom, this.bounds.minBottom, this.bounds.maxBottom)
          break
        default:
          break
      }
    ...
    // 记录操作前的位置信息
    recordPos () {
      if (!this.cachePos?.top) {
        this.cachePos = {}
      }
      this.cachePos.top = this.top
      this.cachePos.right = this.right
      this.cachePos.bottom = this.bottom
      this.cachePos.left = this.left
      this.cachePos.width = this.width
      this.cachePos.height = this.height
      this.$children.map((item) => {
        item.cachePos = {}
        item.cachePos.top = item.top
        item.cachePos.right = item.right
        item.cachePos.bottom = item.bottom
        item.cachePos.left = item.left
        item.cachePos.width = item.width
        item.cachePos.height = item.height
      })
    },
    limitsCheck () {
      ...
      if (children.length) {
        let limits = {
          maxTop: children[0].top,
          maxRight: children[0].right,
          maxBottom: children[0].bottom,
          maxLeft: children[0].left
        }
        children.map(item => {
          limits = {
            maxTop: Math.min(item.top, limits.maxTop),
            maxRight: Math.min(item.right, limits.maxRight),
            maxBottom: Math.min(item.bottom, limits.maxBottom),
            maxLeft: Math.min(item.left, limits.maxLeft)
          }
        })

        const maxLeftChild = children.find(item => item.left === limits.maxLeft).$el
        const maxRightChild = children.find(item => item.right === limits.maxRight).$el
        const maxTopChild = children.find(item => item.top === limits.maxTop).$el
        const maxBottomChild = children.find(item => item.bottom === limits.maxBottom).$el

        const parentEle = this.$el.parentElement

        this.bounds.maxLeft = Math.abs(parentEle.getBoundingClientRect().left - maxLeftChild.getBoundingClientRect().left)
        this.bounds.maxRight = Math.abs(parentEle.getBoundingClientRect().right - maxRightChild.getBoundingClientRect().right)
        this.bounds.maxTop = Math.abs(parentEle.getBoundingClientRect().top - maxTopChild.getBoundingClientRect().top)
        this.bounds.maxBottom = Math.abs(parentEle.getBoundingClientRect().bottom - maxBottomChild.getBoundingClientRect().bottom)
      }
    },
    // 计算调整大小范围
    calcResizeLimits () {
    ...
    async handleUp (e) {
      ...
      removeEvent(document.documentElement, eventsFor.move, this.handleResize)
      // 拖动父级的控制柄，需要计算父级的大小
      this.$children.map(item => item.checkParentSize())
    },
    checkMovePermission () {
      const originEle = this.$parent.$el

      originEle.style.zIndex = '0'

      const parentBounds = originEle.getBoundingClientRect()

      const childBounds = this.$el.getBoundingClientRect()

      // 元素的四个角坐标
      const corners = [
        [childBounds.left, childBounds.top],
        [childBounds.right, childBounds.top],
        [childBounds.left, childBounds.bottom],
        [childBounds.right, childBounds.bottom]
      ]

      const targetEle = document.elementFromPoint(corners[0][0], corners[0][1])

      const upperEles = corners.map(item => document.elementFromPoint(item[0], item[1]))

      const isContained = new Set(upperEles).size === 1

      const isSameKey = originEle.dataset.uniquekey === targetEle.dataset.uniquekey
      ...
      if (isContained && isSameKey) {
        const newParentBounds = targetEle.getBoundingClientRect()
        const borderWidth = parseInt(window.getComputedStyle(targetEle).getPropertyValue('border-width'))
        const offsetX = childBounds.left - newParentBounds.left - borderWidth
        const offsetY = childBounds.top - newParentBounds.top - borderWidth
        const originId = originEle.dataset.id
    ...
    conflictCheck () {
      if (this.isConflictCheck) {
        ...
        const RectA = this.$el.getBoundingClientRect()
        for (let item of nodes) {
          if (item.className !== undefined && !item.className.includes(this.classNameActive) && item.getAttribute('data-is-check') !== null && item.getAttribute('data-is-check') !== 'false') {
            const RectB = item.getBoundingClientRect()
            // 如果冲突，就将回退到移动前的位置
            // 只针对规则矩形有效
            const nonIntersect = (RectB.right < RectA.left) || (RectB.left > RectA.right) || (RectB.bottom < RectA.top) || (RectB.top > RectA.bottom)
            if (!nonIntersect) {
              this.top = this.mouseClickPosition.top
              this.left = this.mouseClickPosition.left
              this.right = this.mouseClickPosition.right
              ...
              if (this.$children.length) {
                this.$children.map(item => {
                  item.top = item.cachePos?.top || item.top
                  item.right = item.cachePos?.right || item.right
                  item.bottom = item.cachePos?.bottom || item.bottom
                  item.left = item.cachePos?.left || item.left
                })
              }
            }
          }
        ...
        for (let item of nodes) {
          if (item.className !== undefined && !item.className.includes(this.classNameActive) && item.getAttribute('data-is-check') !== null && item.getAttribute('data-is-check') !== 'false') {
            ...
            if (tfAndBr || brAndTf || bAndT || tAndB || lAndR || rAndL) {
              this.top = this.cachePos?.top || this.mouseClickPosition.top
              this.left = this.cachePos?.left || this.mouseClickPosition.left
              this.right = this.cachePos?.right || this.mouseClickPosition.right
              this.bottom = this.cachePos?.bottom || this.mouseClickPosition.bottom
              this.width = this.mouseClickPosition.w
              this.height = this.mouseClickPosition.h
              this.$emit('resizing', this.left, this.top, this.width, this.height)
              // 子元素位置复原
              this.$children.map(item => {
                item.top = item.cachePos?.top
                item.right = item.cachePos?.right
                item.bottom = item.cachePos?.bottom
                item.left = item.cachePos?.left
              })
            }
          }
        }
  ...
  watch: {
    ...
    width (val) {
      this.$emit('update:w', val)
    },
    height (val) {
      this.$emit('update:h', val)
    },
    left (val) {
    ...
    enabled (val) {
      if (val) {
        addEvent(this.$el, 'keydown', this.keyboardMove)
      } else {
        removeEvent(this.$el, 'keydown', this.keyboardMove)
        // this.resetBoundsAndMouseState()
      }
    },
    '$parent.width' (oldVal, newVal) {
      if (newVal) {
        const deltaX = newVal - oldVal
        const handle = this.$parent.handle
        if (!handle) {
          return false
...
<style src="./vue-draggable-resizable.css"></style>
