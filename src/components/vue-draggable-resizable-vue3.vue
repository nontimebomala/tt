...
<script>
import { $on, $off, $once, $emit, $children } from 'utils/gogocodeTransfer'
import {
...
export default {
  ...
  methods: {
    ...
    calcDragLimits () {
      if (!$children(this).length) {
        ...
        const parentArray = document.querySelectorAll(
          `[data-uniqueKey="${uniqueKey}"]`
        )
    ...
    deselect (e) {
      ...
      // 点击自身（包括控制柄，不包括子元素）
      const selfClick =
        this.$el === target || target.classList.contains('handle')
    ...
    async keyboardMove (e) {
      ...
      switch (keyCode) {
        ...
        case 39:
          ...
          this.right = restrictToBounds(
            ++this.right,
            this.bounds.minRight,
            this.bounds.maxRighminRight
          )
          break
        case 40:
          this.top = restrictToBounds(
    ...
    limitsCheck () {
      ...
      if (children.length) {
        ...
        const maxLeftChild = children.find(
          (item) => item.left === limits.maxLeft
        ).$el
        const maxRightChild = children.find(
          (item) => item.right === limits.maxRight
        ).$el
        const maxTopChild = children.find(
          (item) => item.top === limits.maxTop
        ).$el
        const maxBottomChild = children.find(
          (item) => item.bottom === limits.maxBottom
        ).$el

        const parentEle = this.$el.parentElement

        this.bounds.maxLeft = Math.abs(
          parentEle.getBoundingClientRect().left -
            maxLeftChild.getBoundingClientRect().left
        )
        this.bounds.maxRight = Math.abs(
          parentEle.getBoundingClientRect().right -
            maxRightChild.getBoundingClientRect().right
        )
        this.bounds.maxTop = Math.abs(
          parentEle.getBoundingClientRect().top -
            maxTopChild.getBoundingClientRect().top
        )
        this.bounds.maxBottom = Math.abs(
          parentEle.getBoundingClientRect().bottom -
            maxBottomChild.getBoundingClientRect().bottom
        )
      }
    },
    // 计算调整大小范围
    ...
    async handleUp (e) {
      ...
      removeEvent(document.documentElement, eventsFor.move, this.handleResize)
      // 拖动父级的控制柄，需要计算父级的大小
      $children(this).map((item) => item.checkParentSize())
    },
    checkMovePermission () {
      ...
      const upperEles = corners.map((item) =>
        document.elementFromPoint(item[0], item[1])
      )

      const isContained = new Set(upperEles).size === 1

      const isSameKey =
        originEle.dataset.uniquekey === targetEle.dataset.uniquekey
      ...
      if (isContained && isSameKey) {
        ...
        const borderWidth = parseInt(
          window.getComputedStyle(targetEle).getPropertyValue('border-width')
        )
        ...
        for (let item of nodes) {
          if (
            ...
            if (tfAndBr || brAndTf || bAndT || tAndB || lAndR || rAndL) {
              ...
              this.bottom =
                this.cachePos?.bottom || this.mouseClickPosition.bottom
              this.width = this.mouseClickPosition.w
              this.height = this.mouseClickPosition.h
              $emit(
                this,
  ...
  watch: {
    ...
    width (val) {
      $emit(this, 'update:w', val)
    },
    height (val) {
      $emit(this, 'update:h', val)
    },
    left (val) {
      $emit(this, 'update:x', val)
    },
    top (val) {
      $emit(this, 'update:y', val)
    },
    enabled (val) {
      if (val) {
